 ...
 * @param {Boolean} [cookies.secure] - Marks the cookies to be used with HTTPS only. Default is false.
 * @param {Boolean} [cookies.httpOnly] - Flags the cookies to be accessible only by the web server. Default is true.
 * @param {Boolean} [cookies.signed] - Indicates if the cookies should be signed. Default is true.
 * @param {Boolean} [cookies.maxAge] - The cookie expiry time relative to the current time in milliseconds. Default is 60000.
...
function LtiLogin({ service, cookies={}, fields=LtiFields.lower, logger }={}) {
  ...
  let usesDeployments = (typeof service.findDeployment === "function")
  ...
  const loginhandler = async function(req, res, next) {
    ...
    try {
      ...
      if (usesDeployments) {
        let deployment = await service.findDeployment(findParams)
        if (!deployment) throw new Error("Unregistered deployment.")  
      }
      ...
      if (logger) logger("LtiLogin redirecting to", url)
...
/**
 * @description Handles LTI 1.1 basic authentication
 * @param {Object}  service - An object implementing the OidcService
 * @param {Object}  fields - Key/value pairs to map LTI fields to application fields. See LtiFields for examples.
 * @returns {Function} - Returns an Express style route handler
 */
function LtiBasic({ service, fields=LtiFields.lower, images="strict", logger } = {}) {
  ...
  const basichandler = async function(req, res, next) {
    ...
    try {
      ...
      // already verified consumerkey above
...
/**
 * @description Detects if launch is 1.1 or 1.3 and uses LtiBasic or LtiVerify
 * @param {Object}  service - An object implementing the OidcService
 * @param {Object}  cookies - Specifies hhow to handle cookies.
 * @param {Boolean} [cookies.secure] - Marks the cookies to be used with HTTPS only. Default is false.
 * @param {Boolean} [cookies.httpOnly] - Flags the cookies to be accessible only by the web server. Default is true.
 ...
 * @returns {Function} - Returns an Express style route handler
 */
function LtiLaunch({ service, cookies={}, fields=LtiFields.lower, images="strict", logger } = {}) {
  const verify = LtiVerify({ service, cookies, fields, images, logger })
  const basic = LtiBasic({ service, cookies, fields, images, logger })
  ...
  const handler = async function(req, res, next) {
    ...
    try {
      ...
      if (logger) logger("LtiLaunch determining launch type")

      // check query params for LTI 1.0/1.1
      if (req.query.lti_version) {
        let ver = req.query.lti_version.toUpperCase()
        if (ver == "LTI-1P0" || ver == "LTI-1P1") 
          return basic(req, res, next)
      }

      // check post params for LTI 1.0/1.1
      if (req.body.lti_version) {
        let ver = req.body.lti_version.toUpperCase()
        if (ver == "LTI-1P0" || ver == "LTI-1P1") 
          return basic(req, res, next)
      }

      // otherwise assume LTI 1.3
