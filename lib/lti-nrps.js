...
class LtiNameRoleService extends LtiAdvantage {
  ...
  constructor({url, clientid, scopes=LtiScopes.members, jwtkey, jwtopts} = {}) {
   ...
   */
  async getMembers({url, role="", limit=100, pages=999} = {}) {
    let next = url
    let curPage = 1
    let result = { members: [] }
  ...
  async getLearners({url, limit=100, pages=999} = {}) {
    return this.getMembers({url, role: "Learner", limit, pages})
  }
  
  async getInstructors({url, limit=100, pages=999} = {}) {
    return this.getMembers({url, role: "Instructor", limit, pages})
  }  
