    ...
    try {
      ...
      switch (platform.keytype) {
        case 'JWK_SET': 
          ...
          if (!pem) {
            let res = await axios.get(platform.keyval)
          ...
          break
        case 'JWK_KEY': 
          if (!platform.keyval) throw new Error("Platform key not found.")
          if (!platform.keyval.kid) throw new Error("Platform kid not found.")
          keyName = cachePrefix+"pem-"+platform.keyval.kid
          if (cache) {
            pem = await cache.get(keyName)
          }
          if (!pem) {
            pem = rasha.jwk2pem(platform.keyval)
            if (pem && cache) {
              await cache.set(keyName, pem)
            }
          }
